...
1. **Answer tone/personality** — **Done, then expanded into a full Personas + Modes engine** (see `docs/DECISIONS.md` D-009 and D-010). Not just Athena's own tone anymore — a data-driven registry (`app/services/personas.py`, `app/services/modes.py`) of selectable personas (**Athena**, **Meera**, **Smiley**, **Raza**, more addable later without code changes elsewhere) crossed with selectable answer-shape modes (**Answering**, **Explaining**, **Teaching**, **Review**, **Summary**), switchable per conversation and mid-conversation via `GET /v1/personas`/`GET /v1/modes` and dropdowns in the composer toolbar (moved from the header to sit with the input, ChatGPT/Claude/Gemini-style, in a later UI pass). Every persona also now closes with a genuinely relevant follow-up instead of just stopping after the answer, and loosely mirrors the user's own phrasing style within a conversation (no persistent profile — just noticing, each turn, how *this* conversation's user talks).
...
3. **As-needed, unordered** (each gets its own scoping pass and `DECISIONS.md` entry when picked up):
   ...
   - **Packaging/distribution** — ship an installable build (not raw source) that sets up the environment automatically, so a non-technical client can run it. `docker-compose.yml` already covers the "technical user" path; a native installer is a separate, bigger effort. Should also **detect the host's specs (RAM/VRAM/GPU) and suggest which local model to install** (e.g. `qwen2.5:7b-instruct` on a low-end/no-GPU machine vs. `qwen2.5:14b-instruct-q4_K_M` on a 12GB-class GPU) — so low-end PC clients aren't stuck trying to run a model too big for their hardware, and stronger machines aren't stuck on the conservative default. Deliberately deferred, not built now — recorded here so it stays part of the plan.
   - **Vision/multimodal (new, bigger scope)** — e.g. a Unity VR client sends a camera image ("what is this pipe fitting?"), a local vision-capable model identifies it, and the answer is grounded in ingested manuals the same way text RAG is today. Genuinely a new capability, not a tweak: needs a local multimodal model (different from the current text-only LLM adapter), a new endpoint accepting an image + question, and Unity-side capture/upload. Record here now so it isn't lost; scope and build it as its own deliberate phase, not folded into this list casually.
   - **Mobile client considered, deliberately not pursued for now**: on-device inference (a mobile-native runtime like llama.cpp + a small quantized model bundled as an Android OBB asset, the approach a student demo used) is technically real but is a genuinely separate mobile engineering project — different language/runtime than this Python stack, a much smaller model than even the "low-end PC" tier, and its own on-device vector search. Not the same problem as low-end *PC* support (already covered by the model-size-detection item above). Revisit only as its own initiative if a real need shows up.
   - **LAN-accessible web frontend** — already works today: the server binds `--host 0.0.0.0`, so another device on the same network can reach it at the host machine's LAN IP (e.g. `http://192.168.x.x:3636`) once a firewall rule allows inbound traffic on that port. Refinements for later, not built yet: (a) during dev, open the firewall port only while the server is actually running and close it again on shutdown, rather than leaving a standing rule; (b) final/installer builds need a properly scoped firewall rule (not wide-open), set up as part of installation; (c) the server should try the next available port automatically if its default port is already in use, instead of failing to start.
   - **Address the user by name/nickname** (Jarvis-calling-Tony-"Tony" energy) — makes responses read as genuinely personal rather than generic, complementing the persona work in item 1. Deliberately scoped **lighter** than the "real per-user logins" bullet above: doesn't need real auth to land. Concretely: the composer gets a simple "what should I call you?" prompt/setting (stored client-side, e.g. `localStorage`), sent as an optional field alongside `persona`/`mode` on `/v1/chat`, and threaded into `_build_system_prompt()` (`app/services/rag.py`) the same way the current date/time is already injected as a stated fact — "the user you're speaking with goes by `<name>`; address them by it naturally, the way each persona would (not every sentence — like a real person would use a name)." Once real per-user logins are built, this graduates from a client-supplied field to the stored `User.display_name`, same integration point.
