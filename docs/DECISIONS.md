...
## D-014 — Explicit resource management: CPU-default for small local models, configurable keep_alive, CPU thread cap · Status: accepted

- **Context:** Raised directly by the user while reviewing GPU-cost-optimization research for the upcoming voice work: nothing in this codebase currently coordinates GPU/CPU usage across the several local models involved (LLM via Ollama, embedder, reranker, and soon STT/TTS) — each independently auto-detects hardware with no awareness of what else is loaded. Concretely, `SentenceTransformerEmbedder` and `CrossEncoderReranker` never passed a `device` to `sentence-transformers`, which silently grabs CUDA whenever it's present. On a single shared GPU (e.g. one 12GB card), that means the embedder and reranker could already be occupying VRAM the LLM needs, before the planned GPU-tier voice engine (Indic Parler-TTS, D-013) even enters the picture — a real risk of OOM or silent fallback to slow CPU, not a hypothetical one.
- **Decision:** Three changes, all deliberately conservative (pin explicit behavior, don't attempt automatic hardware detection/orchestration — that's a bigger future effort, e.g. the already-backlogged installer hardware-detection item in `docs/PLAN.md`):
  1. `EMBEDDING_DEVICE` / `RERANKER_DEVICE` (both default `"cpu"`) — these are small models (~130MB / ~80MB); CPU inference is fast enough that reserving GPU memory exclusively for the LLM (and later, GPU-tier TTS) is the right default. Override per-deployment only if there's GPU headroom to spare.
  2. `LLM_KEEP_ALIVE` (default `"5m"`, Ollama's own default made explicit rather than implicit) — controls how long the model stays resident in VRAM after the last request. Now a deliberate, documented tradeoff (shorter frees VRAM sooner for other local models at the cost of reload latency; longer avoids reload latency but holds VRAM while idle) instead of an invisible default nobody chose.
  3. `CPU_THREAD_LIMIT` (default unset = torch's own default, usually all logical cores) — pinned via `torch.set_num_threads()` at process start (`app/main.py`, before any torch-based model loads) if set. Exists for lower-end/low-core-count hardware, where unbounded CPU-bound model inference can starve FastAPI's own event loop — same "low-end PC" concern already on the Phase 4 backlog.
- **Consequences:** No behavior change for the common case (a single GPU running just the LLM) beyond embedder/reranker now explicitly using CPU instead of implicitly grabbing GPU if present — worth confirming this doesn't regress embedding/rerank latency meaningfully on the dev machine (not expected to, given the model sizes). This is explicit control, not automatic orchestration: nothing here detects free VRAM at runtime and decides for the user. The voice plan's Indic Parler-TTS `device="auto"` setting should be revisited to also check free VRAM (`torch.cuda.mem_get_info()`), not just CUDA availability, before that adapter is actually built — noted, not yet done.
